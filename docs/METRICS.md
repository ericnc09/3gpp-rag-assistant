...
**Evaluated:** 2026-07-02 (full index, `--full --judge`); retrieval metrics identical to the committed 2026-06-25 baseline (`data/eval/baseline.json`)  
...
| Answer-layer refusal (manual, 2026-07-02) | 5/5 (100%) | Every probe answer explicitly declines (see `answer_preview` in the artifact). The automated `is_refusal()` detector scored 0/5 — its phrase list misses the observed wording; fix tracked in `PHASE2.md` Track B |
...
### Answer quality metrics (measured 2026-07-02)
...
|---|---|---|---|
| Faithfulness (heuristic: grounding signal + context overlap) | >80% | **0.834** | Met |
| LLM-judge faithfulness (N=30) | none set | **0.68** | Judge credits grounding, flags missing detail |
| LLM-judge answer correctness (N=30) | none set | **0.40** | Includes 5 refusal probes judged 0 by design; in-corpus pattern is "correct but lacks specific details" — a responder-size ceiling |
| Composite answer score | >0.70 | **0.534** | Missed — deflated by the answer-relevance harness gap below |
| Answer relevance (keyword match in response) | >70% | not computed | Harness defect: metric reads an `answer_keywords` field the golden set lacks; every case scored 0.0. Fix tracked in `PHASE2.md` Track B |
...
| LLM-judge faithfulness | Judge model assesses whether each answer is grounded in retrieved context | Measured 2026-07-02: **0.68** (local-path responder) |
| LLM-judge answer correctness | Judge model assesses factual accuracy within the context window | Measured 2026-07-02: **0.40** (local-path responder; refusal probes judged 0 by design) |
...
| Retrieve latency p50 | 0.021s | Clean retrieval-only run (2026-06-25 baseline); 0.022s under concurrent generation load (2026-07-02) |
| Generate latency p50 (Groq) | `[RUN REQUIRED]` | Groq llama-3.3-70b; depends on response length and rate limits |
| Generate latency p50 (Ollama/llama3.2, CPU) | **44.3s** (p95 69.8s) | Measured 2026-07-02 with full RAG prompts; the earlier "~2–4s" estimate was wrong and is retracted |
| End-to-end latency p50 (local path) | **44.3s** | Retrieval is negligible; generation dominates. **The <5s end-to-end target is not met on the local CPU path.** The cloud path (Groq) is unmeasured and is where the target realistically lives |
